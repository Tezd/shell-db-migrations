...
${CLICKHOUSE_CONNECTION} --query "insert into migration values(${MIGRATION_ID_THIRD}, now())"

MIGRATION_OUTPUT=$(MIGRATION_DIR=${BASE_MIGRATION_DIR} ADAPTER=clickhouse ${BASE_DIR}/migrate.sh ${ARGS})
